/**
 * Advertisers › Bulk Edit — full-page flow (Everflow-style: dual-list advertiser picker + a dynamic
 * list of field changes, reviewed before applying). Real: loops PATCH /api/advertisers/:id per
 * selected advertiser with the merged set of changes, after a review step.
 */
...
type FieldType = 'status' | 'accountManagerId' | 'salesManagerId' | 'billingFrequency' | 'defaultCurrency';

const FIELD_OPTIONS: { value: FieldType; label: string }[] = [
  { value: 'status', label: 'Status' },
  { value: 'accountManagerId', label: 'Account Manager' },
  { value: 'salesManagerId', label: 'Sales Manager' },
  { value: 'billingFrequency', label: 'Billing Frequency' },
  { value: 'defaultCurrency', label: 'Default Currency' },
];
...
function ChangeRow({
  ...
  const renderValueInput = () => {
    ...
    if (change.field === 'accountManagerId' || change.field === 'salesManagerId') {
      return (
        <select className="input" value={change.value} onChange={(e) => onChange({ ...change, value: e.target.value })}>
          <option value="">Select…</option>
    ...
    return <input className="input" maxLength={3} value={change.value} onChange={(e) => onChange({ ...change, value: e.target.value.toUpperCase() })} />;
  };
...
export default function AdvertisersBulkEdit() {
  const nav = useNavigate();
  const location = useLocation();
  const initialIds = (location.state as { selectedIds?: string[] } | null)?.selectedIds ?? [];
  const { data: advertisers, loading } = useQuery<Advertiser[]>('/api/advertisers');
  ...
  const filteredAdvertisers = useMemo(() => {
    ...
    if (!qq) return advertisers ?? [];
    return (advertisers ?? []).filter((a) => a.name.toLowerCase().includes(qq));
  }, [advertisers, q]);
  const available = filteredAdvertisers.filter((a) => !selectedIds.has(a.id));
  ...
  const selectAllAvailable = () => setSelectedIds((s) => new Set([...s, ...available.map((a) => a.id)]));
  ...
  return (
    <>
      <PageHeader title="Bulk Edit" subtitle="Advertisers › Bulk Edit" />
      {!reviewing ? (
        <div className="space-y-4">
          <div className="card">
            <h3 className="mb-3 text-h3 font-medium text-fg">Advertiser Selection</h3>
            ...
            <div className="grid grid-cols-2 gap-3 rounded-card border border-border">
              <div className="border-r border-border">
                ...
                <div className="max-h-72 overflow-y-auto p-2">
                  {available.map((a) => (
                    <button key={a.id} type="button" onClick={() => toggleSelect(a.id)}
                  ...
                  {available.length === 0 && <p className="px-2 py-3 text-small text-fg-muted">No advertisers.</p>}
                </div>
              </div>
              <div>
                <div className="flex items-center justify-between border-b border-border bg-page px-3 py-2">
                ...
                <div className="max-h-72 overflow-y-auto p-2">
                  ...
                  {selected.length === 0 && <p className="px-2 py-3 text-small text-fg-muted">No advertisers selected.</p>}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-end gap-2">
            <Link to="/app/advertisers" className="btn-ghost">Cancel</Link>
        ...
        <div className="card space-y-4">
          ...
          <p className="text-small text-fg-secondary">
            Applying <strong>{validChanges.length}</strong> change{validChanges.length === 1 ? '' : 's'} to <strong>{selectedIds.size}</strong> advertiser{selectedIds.size === 1 ? '' : 's'}.
          </p>
          <ul className="space-y-1 text-small text-fg">
            {validChanges.map((c) => (
              <li key={c.id}>{FIELD_OPTIONS.find((f) => f.value === c.field)?.label}: <strong>{c.field === 'accountManagerId' || c.field === 'salesManagerId' ? userName(c.value) : c.value}</strong></li>
            ))}
          </ul>
