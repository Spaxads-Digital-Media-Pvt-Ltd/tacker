 ...
 *
 * 5. Graceful degradation: when ClickHouse is disabled, the singleton in `index.ts` keeps
 * the Postgres provider. This file is only loaded when `isClickHouseEnabled()` is true.
 */
...
export class ClickHouseReportingProvider implements ReportingProvider {
 ...
 // ClickHouse DateTime64 does not accept ISO 8601 strings with the `Z` suffix; parse via
 // parseDateTime64BestEffort at the boundary. Both the literal and the comparison column are
 // cast so the optimizer can use the index.
 const phTs = (v: unknown) => { params.push(v); return `parseDateTime64BestEffort({p${params.length}:String}, 3)`; };

 // WHERE clauses — identical filter semantics to Postgres provider.
 ...
 // Include filters: each filter applies to both CTEs.
 ...
 // "Exclude Invalid": drop fraud-flagged clicks from click-side metrics.
 if (f.excludeInvalid) clickWhere.push('length(c.fraud_flags) = 0');

 const gbHas = gb.length > 0;
 const groupIdx = gb.map((_, i) => `d${i}`).join(', ');

 // The outer SELECT below references every click-side and conversion-side metric alias
 // unconditionally (so the same query shape handles any metric subset). Therefore the CTEs
 // must also expose every alias — not just the ones the caller asked for. Without this,
 // `cl.unique_clicks` etc. are "unknown identifier" errors in ClickHouse.
 ...
 // Conversion CTE — conversions already denormalized, no JOIN needed.
 ...
 // Outer join to combine click and conversion aggregates.
 // Cast dimensions to String inside COALESCE so UUID (cl.offer_id) and String (\0 sentinel)
 // have a common supertype — ClickHouse refuses `COALESCE(UUID, String)` directly.
 ...
 // Derived metrics computed after the join.
 ...
 // Default sort: day/hour in chronological order; otherwise clicks descending.
 // Matches PostgresReportingProvider exactly: chronological defaults to ASC only when
 // no explicit orderBy is set.
...
/**
 * Install the ClickHouse-backed reporting provider if ClickHouse is configured.
 * Otherwise the existing Postgres provider stays in place.
 */
export function installClickHouseReportingProvider(): boolean {
 if (!isClickHouseEnabled()) {
 logger.info('clickhouse not configured — reporting provider stays Postgres');
 return false;
 }
 setReportingProvider(new ClickHouseReportingProvider());
 logger.info('clickhouse reporting provider installed');
 return true;
}
