/**
 * Reporting › Hourly — verified item-by-item against the live reference. Same shape as Daily
 * Report (rows are time buckets, not entities — no expand, no "View X", just Open Flex Report in
 * the row kebab) but grouped by `date_trunc('hour', created_at)` instead of day, and with two extra
 * columns the reference shows here that Daily doesn't: "cc" (no real backing anywhere in this app,
 * shown as "—") and "fraud" (real — the same avg 0-100 `clicks.fraud_score` already used on the
 * Offer Report). Shares its Summary tile grid, "Reporting Filters" flyout, and page-level kebab with
 * the other report pages — see components/ReportPageKit.tsx.
 *
 * `hour` was already a supported reporting dimension. The chronological-default-sort addition made
 * for the Daily Report already covers `hour` too (see api-backend/src/lib/reporting/postgres.ts).
 *
 * Gross Clicks / Clicks (net of fraud-flagged) / Dup./Invalid Clicks, Total CV/CV, CVR/CPC/CPA/RPC/
 * RPA, Revenue/Payout/Profit/Margin/Fraud are all real. Impression/RPM/CPM/VT CV/CTR/Throttle/CC
 * have no real source anywhere in this app — shown as "—" rather than a fabricated value.
 */
...
const ALL_COLUMNS = [
  ...
  'Revenue', 'Payout', 'Profit', 'Margin', 'CC', 'Fraud',
] as const;
...
export default function HourlyReport() {
  ...
  const [from, setFrom] = useState(searchParams.get('from') ?? daysAgo(7));
  ...
  const [savedReports, setSavedReports] = useState(() => loadSavedReports<OrderMetric>('hourly-report'));
  ...
  const summaryQs = qs({ groupBy: 'hour', metrics: METRICS_PARAM, from: toIso(appliedFrom), to: toIso(appliedTo, true), ...dimParams, limit: 200 });
  ...
  const graphQs = qs({ groupBy: 'hour', metrics: 'clicks,revenue', from: toIso(appliedFrom), to: toIso(appliedTo, true), ...dimParams, limit: 200 });
  const { data: graphData } = useQuery<AggResult>(graphOpen ? `/api/reports?${graphQs}` : null);
  const graphSeries = useMemo(() => {
    const rows = [...(graphData?.rows ?? [])].sort((a, b) => (a.dimensions['hour'] ?? '').localeCompare(b.dimensions['hour'] ?? ''));
    return {
      labels: rows.map((r) => (r.dimensions['hour'] ?? '').slice(0, 13)),
  ...
  const tableQs = qs({
    groupBy: 'hour', metrics: METRICS_PARAM,
  ...
  const rows = useMemo(() => (data?.rows ?? [])
    .filter((r) => r.dimensions['hour'])
    .map((r) => ({ hour: r.dimensions['hour']!, derived: deriveRow(r.metrics) }))
    .filter((r) => !q.trim() || formatHour(r.hour).includes(q.trim()))
  ...
  const clearAll = () => {
    setFrom(daysAgo(7)); setTo(todayStr()); setFilters({}); setExclusions({}); setMetricFilters({}); setIgnoreFailTraffic(false);
  ...
  const exportRows = () => rows.map((r) => ({
    hour: formatHour(r.hour), imp: DASH, rpm: DASH, cpm: DASH, grossClicks: r.derived.clicksGross, clicks: r.derived.clicks,
    ...
    margin: pct(r.derived.marginPct), cc: DASH, fraud: r.derived.fraudScore.toFixed(0),
  }));
  ...
  const saveReport = () => {
    ...
    persistSavedReports('hourly-report', next);
  ...
  return (
    <>
      <PageHeader title="Hourly Report" subtitle="Reporting › Hourly" action={
        ...
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input className="input !w-56 !pl-8" placeholder="Search hours…" value={q} onChange={(e) => setQ(e.target.value)} />
            </div>
                  ...
                  <div className="relative" onMouseEnter={() => setExportOpen(true)} onMouseLeave={() => setExportOpen(false)}>
                    ...
                    {exportOpen && (
                      <div className="absolute right-full top-0 mr-1 w-32 rounded-card border border-border bg-elevated py-1 shadow-elevated">
                        <button onClick={() => { downloadCsv('hourly-report.csv', exportRows()); setTableActionsOpen(false); setExportOpen(false); }} className="block w-full px-3 py-1.5 text-left text-small text-fg hover:bg-accent-subtle">CSV</button>
                        <button onClick={() => { downloadXlsx('hourly-report.xlsx', exportRows()); setTableActionsOpen(false); setExportOpen(false); }} className="block w-full px-3 py-1.5 text-left text-small text-fg hover:bg-accent-subtle">Excel</button>
                      </div>
                    )}
                  </div>
        ...
        {loading ? <StateBlock><Spinner /></StateBlock>
          ...
          : (
            <div className="overflow-x-auto rounded-card border border-border">
              <table className="w-full min-w-[2100px] text-left text-body">
                <thead className="border-b border-border bg-page text-tiny uppercase tracking-wide text-fg-secondary">
                  <tr>
                    <th className="whitespace-nowrap px-4 py-3 font-semibold">Hour</th>
                ...
                <tbody className="divide-y divide-border">
                  {rows.map((r) => (
                    <tr key={r.hour} className="hover:bg-accent-subtle/40">
                      <td className="whitespace-nowrap px-4 py-3 font-medium text-fg">{formatHour(r.hour)}</td>
