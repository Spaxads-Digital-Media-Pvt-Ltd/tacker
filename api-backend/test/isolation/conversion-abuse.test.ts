...
describe('checkConversionAbuse (T-2)', () => {
 ...
 const result = await checkConversionAbuse(redis, 'click-1', 'txn-1', 'net-1');
 ...
 for (let i = 0; i < 10; i++) {
 const r = await checkConversionAbuse(redis, 'click-1', `txn-${i}`, 'net-1');
 expect(r.limited).toBe(false);
 }
 // 11th call: count = 11 > 10
 const r = await checkConversionAbuse(redis, 'click-1', 'txn-last', 'net-1');
 ...
 await checkConversionAbuse(redis, 'click-1', `txn-${i}`, 'net-1');
 }
 ...
 const r1 = await checkConversionAbuse(redis, 'click-1', 'same-txn', 'net-1');
 expect(r1.limited).toBe(false);
 const r2 = await checkConversionAbuse(redis, 'click-1', 'same-txn', 'net-1');
 ...
 const redis = makeRedis();
 const result = await checkConversionAbuse(redis, 'click-1', null, 'net-1');
 expect(result.limited).toBe(false);
 ...
 const redis = makeRedis();
 const result = await checkConversionAbuse(redis, 'click-1', '', 'net-1');
 expect(result.limited).toBe(false);
 expect(result.count).toBe(0);
 expect(redis.incr).not.toHaveBeenCalled();
 });
 ...
 const counts: Record<string, number> = {};
 const redis = makeRedis(counts);
 await checkConversionAbuse(redis, 'click-A', 'txn-1', 'net-1');
 await checkConversionAbuse(redis, 'click-A', 'txn-2', 'net-1');
 await checkConversionAbuse(redis, 'click-B', 'txn-1', 'net-2');
 ...
 const counts: Record<string, number> = {};
 const redis = makeRedis(counts);
 await checkConversionAbuse(redis, 'click-1', 'txn-1', 'net-1');
 await checkConversionAbuse(redis, 'click-1', 'txn-2', 'net-1');
 await checkConversionAbuse(redis, 'click-1', 'txn-3', 'net-1');
 ...
 const counts: Record<string, number> = {};
 const redis = makeRedis(counts);
 await checkConversionAbuse(redis, 'click-abc', 'txn-xyz', 'net-999');
