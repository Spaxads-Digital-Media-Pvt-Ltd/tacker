        ...
        : status.data && !status.data.configured ? (
          <div className="card border-dashed">
            ...
            <p className="mt-2 text-sm text-fg-secondary ">
              Add <code className="font-mono text-xs">ANTHROPIC_API_KEY</code> to the backend environment to enable the AI ops assistant. Everything else is ready.
            </p>
          </div>
        ) : (
          <>
            <div className="flex-1 space-y-4 overflow-y-auto rounded-2xl border border-border bg-surface/50 p-4 ">
              {messages.length === 0 && (
                <div className="grid h-full place-items-center">
                  <div className="max-w-md text-center">
                    ...
                    <p className="mt-3 text-sm text-fg-secondary ">Ask anything about your network. Try one of these:</p>
                    <div className="mt-4 grid gap-2">
                      {SUGGESTIONS.map((s) => (
                        <button key={s} onClick={() => void send(s)} className="rounded-lg border border-border px-3 py-2 text-left text-sm hover:border-brand-400 ">{s}</button>
                      ))}
                    </div>
                  </div>
                </div>
              ...
              {messages.map((m, i) => (
                <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm ${m.role === 'user' ? 'bg-brand-600 text-white' : 'border border-border bg-surface '}`}>
                    <div className="whitespace-pre-wrap">{m.content}</div>
                    {m.tools && m.tools.length > 0 && (
                      <div className="mt-2 flex flex-wrap gap-1">
                        {m.tools.map((t, j) => <span key={j} className="rounded bg-subtle px-1.5 py-0.5 text-[10px] font-medium text-fg-secondary ">{t}</span>)}
                      </div>
                    )}
                  </div>
                </div>
