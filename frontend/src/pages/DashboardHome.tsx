...
import { InteractiveSingleSeriesChart } from '../components/InteractiveSingleSeriesChart';
                ...
                {visible.kpi && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 mb-6">
                    ...
                    <div className="animate-slide-in-left h-full" style={{ animationDelay: '220ms' }}>
                      <Kpi label="Margin" icon={PiChartPieSliceDuotone} iconColor="text-purple-500" value={money(data.margin.today)} series={data.series.revenue}
...
function Kpi({ label, icon: Icon, iconColor = 'text-accent', value, series, d, rows, className = '' }: {
...
function InteractiveKpi({ label, icon: Icon, iconColor = 'text-accent', value, series, d, rows, className = '' }: {
  label: string; icon: React.ElementType; iconColor?: string; value: string; series: number[];
  d: { pct: number; up: boolean } | null; rows: [string, string][];
  className?: string;
}) {
