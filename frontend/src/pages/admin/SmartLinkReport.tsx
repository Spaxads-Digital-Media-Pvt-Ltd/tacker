...
function ExpandedOfferRows({ smartLinkId, colSpanBefore, shown }: { smartLinkId: string; colSpanBefore: number; shown: Set<string> }) {
  const { data: offers } = useQuery<Offer[]>('/api/offers');
  const { data, loading } = useQuery<AggResult>(`/api/reports?groupBy=offer&metrics=${METRICS_PARAM}&smartLinkId=${smartLinkId}&limit=200`);
...
export default function SmartLinkReport() {
  ...
  const urlSmartLinkId = searchParams.get('smartLinkId') ?? '';
  ...
  const initialFilters: FilterValues = urlSmartLinkId ? { smartLink: [urlSmartLinkId] } : {};
  ...
  useEffect(() => {
    ...
    if (smartLinkIdFilter) next.set('smartLinkId', smartLinkIdFilter);
  ...
  }, [appliedFrom, appliedTo, smartLinkIdFilter, offerIdFilter, countryFilter, deviceFilter, orderBy, orderDir]);
  ...
  const summaryQs = qs({ groupBy: 'smartLink', metrics: METRICS_PARAM, from: toIso(appliedFrom), to: toIso(appliedTo, true), ...dimParams, limit: 200 });
  ...
  const rows = useMemo(() => (data?.rows ?? [])
    .filter((r) => r.dimensions['smartLink'])
    .map((r) => {
      const smartLink = smartLinks?.find((s) => s.id === r.dimensions['smartLink']);
      return { smartLinkId: r.dimensions['smartLink']!, smartLinkName: smartLink?.name ?? r.dimensions['smartLink']!.slice(0, 8), smartLinkStatus: smartLink?.status, derived: deriveRow(r.metrics) };
    })
  ...
  [data, smartLinks, q, appliedMetricFilters]);
        ...
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input className="input !w-56 !pl-8" placeholder="Search smart links…" value={q} onChange={(e) => setQ(e.target.value)} />
            </div>
                  ...
                  <div className="relative" onMouseEnter={() => setExportOpen(true)} onMouseLeave={() => setExportOpen(false)}>
                    ...
                    {exportOpen && (
                      <div className="absolute right-full top-0 mr-1 w-32 rounded-card border border-border bg-elevated py-1 shadow-elevated">
                        <button onClick={() => { downloadCsv('smartlink-report.csv', exportRows()); setTableActionsOpen(false); setExportOpen(false); }} className="block w-full px-3 py-1.5 text-left text-small text-fg hover:bg-accent-subtle">CSV</button>
                        <button onClick={() => { downloadXlsx('smartlink-report.xlsx', exportRows()); setTableActionsOpen(false); setExportOpen(false); }} className="block w-full px-3 py-1.5 text-left text-small text-fg hover:bg-accent-subtle">Excel</button>
                      </div>
                    )}
                  </div>
                    ...
                    <Fragment key={r.smartLinkId}>
                      <tr className="hover:bg-accent-subtle/40">
                        ...
                        <td className="text-right">
                          <RowActionMenu smartLink={{ id: r.smartLinkId, name: r.smartLinkName }} />
                        </td>
                      </tr>
                      {expanded.has(r.smartLinkId) && <ExpandedOfferRows smartLinkId={r.smartLinkId} colSpanBefore={1} shown={shown} />}
                    </Fragment>
                  ))}
                </tbody>
              </table>
